    ...
    <>
      ...
      <main>
        <section className="bg-[#0f2e36] py-20 text-white sm:py-28">
          <Container>
            <p className="animate-fade-in-up text-sm font-bold uppercase tracking-[.18em] text-[#e8d9cc]">Produits &amp; équipements</p>
            <h1 className="animate-fade-in-up mt-4 max-w-3xl text-4xl font-semibold tracking-tight sm:text-5xl [animation-delay:100ms]">Des solutions professionnelles pour vos besoins d’hygiène.</h1>
          </Container>
        </section>
          ...
          <Container>
            ...
            {published.length ? (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {published.map((product, index) => <Reveal key={product.id} delayMs={index * 80}><ProductCard product={product} isMock={productsAreMocked} /></Reveal>)}
              </div>
